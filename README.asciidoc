...
What's next? In this article we've looked briefly at building a web service with Spring Boot. We looked at Reactor, Spring Data Kay, Spring Framework 5 and Spring WebFlux, Spring Security 5, and Spring Boot 2.0. Spring Boot 2 makes it easy to assemble the various reactive Spring projects into an application. We didn't look at the Spring Boot Actuator in this article, but it serves to support operational concerns in the production application. It surfaces metrics, application health and much more. It took has been updated to work seamlessly in a reactive world.
